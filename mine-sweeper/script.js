...
let highestScore = document.getElementById('leaderboardValue');

const explosionSound = new Audio('./explosion-91872.mp3');
explosionSound.volume = 0.5;

const checkBoundries = (x, y) => (x >= 0 && x < gameSet.cols && y >= 0 && y < gameSet.rows);
const zeroPad = (num, places) => String(num).padStart(places, '0');

document.getElementById('bigContainer').addEventListener('contextmenu', (e) => e.preventDefault());
...
function renderGrid(){
    ...
    gameContainer.style.marginTop = "5px"
    for (let i in gameGrid){
        ...
        for (let j in gameGrid[i]){
            ...
            block.onmousedown = () => restartBtn.innerHTML = ' 😮 ';
            block.onmouseup = () => restartBtn.innerHTML = ' 🙂 ';
...
function disableRevealMines(){
    document.querySelectorAll('.gridBlock').forEach((block) => {
        ...
        if(block.dataset.value == 'M'){
            block.classList.add('gridMine');
            block.innerHTML = `<img src="./mnissr0w.png">`
        }
    });
...
function updateScore(){
    ...
    if(highScores){
        if(difficultyLevel == 'custom'){
            ...
            let diffScore = highScores[difficultyLevel];
            if(gameWon && score < diffScore){
                highScores[difficultyLevel] = score;
                highestScore.innerHTML = zeroPad(score,3);
            }else{
                highestScore.innerHTML = zeroPad(diffScore,3);
            }
        }
    }else{
        highScores = {
...
function generateGame(rows, cols){
    ...
    while(mines > 0) {
        let x = parseInt(Math.random()*1000 % cols ) ;
        let y = parseInt(Math.random()*1000 % rows ) ;
